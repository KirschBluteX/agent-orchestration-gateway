...
2. Primary 只询问会影响行为、范围或责任边界的关键问题，并检查足以确定所有权的仓库上下文。
3. Primary 用一张表展示模块、依赖关系、写入范围、模型与思考强度、子代理上限及审查策略。
   用户确认前不会派遣任务。
4. 无状态校验器检查计划结构，包括依赖环和所有跨模块范围重叠。
...
确认表可以覆盖模块根任务的模型。一项模块根据真正独立的工作使用零至八个叶子代理，上限
不是配额。同一范围的修正会复用原代理。只有涉及安全、并发、持久化、公开契约、安装、
破坏性操作或广泛语义变化时，才增加一名独立审查代理。
...
不安全的仓库相对路径、冗余或跨模块重叠范围、未知依赖和依赖环。`exact` 表示文件，`prefix`
表示目录。校验器输出确定性的规范化 JSON，不读取或写入仓库状态。
