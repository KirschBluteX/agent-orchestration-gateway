...
- CCO remains pre-1.0: its plain `0.9.3` identity has no build metadata, and a minor release may
  make breaking changes. The historical 2.x through 5.x labels are pre-0.9 development history;
  Git history remains unchanged.
- Validate clean 0.9.3 installation, Hook trust, and the breaking-state cleanup path on current
