...
Use GitHub's private vulnerability reporting for this repository. Include the AOG version,
...
AOG is a local workflow guardrail, not an authentication system or OS security boundary. Primary,
...
worktrees; dirty Git and directory workspaces use bounded copies. Before integration AOG records
...
Current state uses `aog.wave.v1`, `aog.lifecycle.v1`, and `aog.receipt.v1`. State can reveal
repository paths, objectives, routes, changed paths, and acceptance evidence; protect the local
state directory accordingly.

AOG ships five exact Hook definitions. It has no global all-tool matcher. Hooks bind native calls
...
ciphertext instead of plaintext. AOG requires an exact, unique match of every visible field to a
...
`AOG_OPAQUE_MESSAGE_POLICY=strict` before starting Codex when that host trust is unacceptable;
...
and rejects missing, duplicate, or unknown AOG definitions; it never repairs or trusts a Hook on
the user's behalf.
