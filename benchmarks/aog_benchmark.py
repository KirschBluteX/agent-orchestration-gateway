#!/usr/bin/env python3
"""Build reproducible AOG benchmark plans without invoking a model."""
...
MANIFEST_PROTOCOL = "aog.benchmark-manifest.v1"
PLAN_PROTOCOL = "aog.benchmark-plan.v1"
USAGE_PROTOCOL = "aog.benchmark-usage.v1"
RESULT_PROTOCOL = "aog.benchmark-result.v1"
SUMMARY_PROTOCOL = "aog.benchmark-summary.v1"
...
def validate_manifest(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for arm in arms:
        ...
        arm_ids.add(arm_id)
        if arm.get("mode") not in {"primary_only", "aog_static"}:
...
def summarize(plan: Mapping[str, Any], results_dir: Path) -> dict[str, Any]:
    ...
    for run in runs:
        ...
        if arm_mode not in {"primary_only", "aog_static"}:
    ...
    aog_arms = sorted(arm_id for arm_id, mode in arm_modes.items() if mode == "aog_static")
    for primary_arm in primary_arms:
        for aog_arm in aog_arms:
            pair_id = f"{primary_arm}__vs__{aog_arm}"
            stats = {
                "aog_static_wins": 0,
                "pairs": 0,
                "primary_only_wins": 0,
            ...
            for cell in grouped.values():
                if primary_arm not in cell or aog_arm not in cell:
                    continue
                stats["pairs"] += 1
                primary_pass = cell[primary_arm]["verdict"] == "pass"
                aog_pass = cell[aog_arm]["verdict"] == "pass"
                if aog_pass and not primary_pass:
                    stats["aog_static_wins"] += 1
                elif primary_pass and not aog_pass:
                    stats["primary_only_wins"] += 1
                else:
                    stats["ties"] += 1
            paired["by_pair"][pair_id] = stats
            for field in ("aog_static_wins", "pairs", "primary_only_wins", "ties"):
...
def record_result(
    ...
    *,
    ...
    return {
        "protocol": "aog.benchmark-receipt.v1",
...
def preflight(
    ...
    *,
    ...
    return {
        ...
        "checks": checks,
        "protocol": "aog.benchmark-preflight.v1",
        "ready": not blockers,
        "started_run": False,
        "study_id": manifest.get("study_id"),
    }
...
def discover_rollouts(codex_home: Path, root_thread_id: str) -> list[Path]:
    ...
    with closing(sqlite3.connect(uri, uri=True)) as connection:
        ...
        for row in children:
            ...
            if row["agent_role"] not in AOG_ROLES:
                raise BenchmarkError(f"child {child_id} is not AOG-owned")
            ...
            if nested is not None:
                raise BenchmarkError(f"AOG child {child_id} delegated another Agent")
