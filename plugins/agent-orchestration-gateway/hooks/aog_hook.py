#!/usr/bin/env python3
"""Exact Codex Hook adapter for the aog.v1 control-plane interface."""
...
def _block(error: Exception | str) -> dict[str, str]:
    return {"decision": "block", "reason": f"AOG: {error}"}
    ...
    if isinstance(
        ...
        if event == "SubagentStop":
            return {
                "decision": "block",
                "reason": (
                    "AOG state is temporarily busy; return the exact same result "
                    "(the same AOG_RESULT) without doing more work."
                ),
            }
        if event == "Stop":
            return {
                "decision": "block",
                "reason": "AOG state is temporarily busy; wait for the lifecycle event.",
            }
    if event == "SubagentStop":
...
def _host_opaque_message(value: object) -> bool:
    ...
    return host_opaque_message(value)
...
def _result_from_transcript(payload: Mapping[str, Any], expected_owner: str) -> str:
    ...
    AOG_RESULT messages from earlier turns.  The terminal tail is ordered; the
    ...
    if latest_candidate is None or not latest_candidate.startswith("AOG_RESULT aog.v1\n"):
        raise ControlPlaneError("native child transcript has no final AOG result")
            ...
            if interrupted:
                return {
                    "hookSpecificOutput": {
                        ...
                        "additionalContext": (
                            "AOG fenced child work interrupted by the host restart. "
            ...
            with deadline_after(PRETOOL_INTERNAL_BUDGET_SECONDS):
                ...
                if tool in SPAWN_TOOLS:
                    ...
                    if not isinstance(message, str) or not message.startswith(
                        ...
                        raise ControlPlaneError(
                            "prepare every native child through the current aog.v1 plan"
                        )
                    _control(value).preflight_spawn(
                        value,
                ...
                if tool in MESSAGE_TOOLS:
                    ...
                    if opaque_message:
                        if tool in FOLLOWUP_TOOLS:
                            _control(value).preflight_opaque_followup(value)
                            return {}
                        ...
                        if isinstance(target, str) and _control(value).owner_is_managed(
                            ...
                            raise ControlPlaneError(
                                "opaque messages cannot bypass a managed AOG continuation"
                            )
                        return {}
                    if _protected(message):
                        raise ControlPlaneError(
                    ...
                    if isinstance(message, str) and message.startswith(
                        ...
                        if tool not in FOLLOWUP_TOOLS:
                            raise ControlPlaneError(
                                "a AOG owner reuse must use followup_task"
                            )
            ...
            with deadline_after(POSTTOOL_INTERNAL_BUDGET_SECONDS):
                ...
                if opaque_message:
                    ...
                    if tool in SPAWN_TOOLS or tool in FOLLOWUP_TOOLS:
                        _control(value).process_postflight_event(
                            value,
                            opaque_message=True,
                        )
                        return {}
                    if tool in MESSAGE_TOOLS:
                        ...
                        if isinstance(target, str) and _control(value).owner_is_managed(
                            ...
                            raise ControlPlaneError(
                                "opaque messages cannot settle a managed AOG continuation"
                            )
                    # An opaque ordinary message to an unmanaged target is not
                    # a AOG lifecycle event.  It was never eligible for
            ...
            with deadline_after(SUBAGENT_STOP_INTERNAL_BUDGET_SECONDS):
                ...
                try:
                    ...
                    return {
                        "continue": False,
                        "systemMessage": (
                            "AOG could not map the native child result and closed its unresolved leases; "
                ...
                try:
                    ...
                    return {
                        "continue": False,
                        "systemMessage": (
                            "AOG rejected and fenced the child result; Primary must inspect the actual state."
                        )
                    }
