#!/usr/bin/env python3
"""Compact aog.v1 plan, wave, routing, and lifecycle control plane."""
...
PROTOCOL = "aog.v1"
PLAN_PROTOCOL = "aog.plan.v1"
WAVE_PROTOCOL = "aog.wave.v1"
BATCH_PROTOCOL = "aog.wave-batch.v1"
LIFECYCLE_PROTOCOL = "aog.lifecycle.v1"
# Receipts are the sole durable record for one-shot lifecycle observations.
PENDING_EVENT_PROTOCOL = "aog.receipt.v1"
TASK_HEADER = "AOG_TASK aog.v1"
CONTINUE_HEADER = "AOG_CONTINUE aog.v1"
RESULT_HEADER = "AOG_RESULT aog.v1"
READ_ROLE = "aog_read_leaf"
WRITE_ROLE = "aog_write_leaf"
...
RECOVERY_FILE_RE = re.compile(r"^\.aog-recovery-[A-Za-z0-9_-]+\.json$")
RECOVERY_STAGING_FILE_RE = re.compile(
    r"^\.aog-staging-[A-Za-z0-9_-]+\.pending$"
)
PENDING_EVENT_FILE_RE = re.compile(
    r"^\.aog-pending-s(?P<session>[0-9a-f]{64})-(?P<event>[0-9a-f]{64})\.event$"
)
STATE_ROOT_SENTINEL = ".aog-state-root-v1"
STATE_ROOT_SENTINEL_BYTES = b"aog.state-root.v1\n"
...
UNSUPPORTED_STATE_MESSAGE = (
    "unsupported AOG state or artifact; remove it only after native work is "
    "inactive, then start a new task"
)
OPAQUE_MESSAGE_POLICY_ENV = "AOG_OPAQUE_MESSAGE_POLICY"
OPAQUE_MESSAGE_POLICIES = frozenset({"strict", "trusted_host"})


class ControlPlaneError(RuntimeError):
    """An aog.v1 contract or lifecycle transition is invalid."""


class ControlPlaneUnavailable(ControlPlaneError):
    """AOG state infrastructure is temporarily unavailable; do not fence work."""
...
def _state_root() -> Path:
    configured = os.environ.get("AOG_STATE_DIR")
    root = (
        Path(configured).expanduser()
        if configured
        else Path(tempfile.gettempdir()) / "agent-orchestration-gateway" / "v1"
    )
...
def _pending_event_path(root: Path, session_id: str, event_id: str) -> Path:
    ...
    return root / (
        f".aog-pending-s{_session_digest(session_id)}-{event_id[7:]}.event"
    )
...
def _write_immutable(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists():
        if _load_object(path, "immutable AOG artifact") != dict(value):
            raise ControlPlaneError("immutable AOG artifact identity collision")
        return
        ...
        with os.scandir(root) as entries:
            for entry in entries:
                ...
                if STATE_FILE_RE.fullmatch(entry.name) is None:
                    raise ControlPlaneError(UNSUPPORTED_STATE_MESSAGE)
...
def _session_state_paths(root: Path, session_id: str) -> tuple[list[Path], bool]:
    """Find current indexed state and detect unsupported lifecycle artifacts."""

    suffix = f"--{_session_digest(session_id)}.json"
    unsupported_name = f"{session_id}.json"
    matches: list[Path] = []
    unsupported = False
    try:
        with os.scandir(root) as entries:
            for entry in entries:
                checkpoint()
                if entry.name == unsupported_name:
                    unsupported = True
    ...
    if isinstance(isolation, Mapping) and isolation.get("mode") == COOPERATIVE:
        body["writer_isolation"] = {
            ...
            "notice": (
                ...
                "workspace_root, which is the AOG-owned isolate. Do not write the "
                "canonical workspace."
            ),
        }
...
def parse_task_message(value: object) -> dict[str, Any]:
    if not isinstance(value, str) or not value.startswith(TASK_HEADER + "\n"):
        raise ControlPlaneError("spawn does not contain an aog.v1 task")
    try:
    ...
    if body.get("protocol") != PROTOCOL or SHA256_RE.fullmatch(str(body.get("dispatch_id"))) is None:
        raise ControlPlaneError("aog.v1 task identity is invalid")
    return body
...
def parse_continue_message(value: object) -> dict[str, Any]:
    if not isinstance(value, str) or not value.startswith(CONTINUE_HEADER + "\n"):
        raise ControlPlaneError("continuation does not contain an aog.v1 contract")
    try:
        body = parse_canonical_json_object(value.split("\n", 1)[1], "aog.v1 continuation")
    except ProtocolHashError as error:
        raise ControlPlaneError(str(error)) from error
    if body.get("protocol") != PROTOCOL or SHA256_RE.fullmatch(str(body.get("dispatch_id"))) is None:
        raise ControlPlaneError("aog.v1 continuation identity is invalid")
    return body


def parse_result(value: object) -> dict[str, Any]:
    if not isinstance(value, str) or not value.startswith(RESULT_HEADER + "\n"):
        raise ControlPlaneError("child did not return an aog.v1 result")
    raw = value.split("\n", 1)[1].strip()
    try:
        result = json.loads(raw, object_pairs_hook=_unique_pairs)
    except (json.JSONDecodeError, ControlPlaneError) as error:
        raise ControlPlaneError("aog.v1 result is not JSON") from error
    required = {
        "blockers",
        "changed_paths",
        "cursor",
    ...
    if not isinstance(result, Mapping) or set(result) != required:
        raise ControlPlaneError("aog.v1 result fields are malformed")
    if SHA256_RE.fullmatch(str(result["dispatch_id"])) is None:
        raise ControlPlaneError("aog.v1 result dispatch identity is invalid")
    cursor = result["cursor"]
    if isinstance(cursor, bool) or not isinstance(cursor, int) or cursor < 0:
        raise ControlPlaneError("aog.v1 result cursor is invalid")
    ...
    if status not in {"complete", "partial", "blocked"} or outcome not in {
        ...
        raise ControlPlaneError("aog.v1 result status or outcome is invalid")
    blockers = result["blockers"]
    deviations = result["deviations"]
    if not isinstance(blockers, list) or not isinstance(deviations, list):
        raise ControlPlaneError("aog.v1 result blockers or deviations are invalid")
    ...
    if len(normalized_blockers) != len(blockers) or len(normalized_deviations) != len(deviations):
        raise ControlPlaneError("aog.v1 result lists contain duplicates")
    ...
    if not isinstance(paths_value, list):
        raise ControlPlaneError("aog.v1 changed_paths must be a list")
    try:
    ...
    if len(changed_paths) != len(paths_value):
        raise ControlPlaneError("aog.v1 changed_paths contains duplicates")
    evidence_value = result["evidence"]
    if not isinstance(evidence_value, Mapping):
        raise ControlPlaneError("aog.v1 evidence must be an object")
...
class ControlPlane:
    """One deep interface for aog.v1 plan, wave, and lifecycle behavior."""
    ...
    def __init__(
        ...
        *,
        ...
        self._uses_default_root = root is None and not os.environ.get("AOG_STATE_DIR")
    ...
    def _state_root_is_marked(self) -> bool:
        ...
        if not marker.exists():
            return False
        if _read_bounded_bytes(marker, "AOG state-root sentinel", limit=128) != (
            STATE_ROOT_SENTINEL_BYTES
        ):
            raise ControlPlaneError("AOG state-root sentinel is invalid")
        return True

    def _mark_state_root_if_safe(self) -> None:
        """Mark only a dedicated or empty state root as AOG-owned."""
            ...
            for path in json_files:
                try:
                    state = self._validate_lifecycle_state(
                        _load_object(path, "current aog.v1 lifecycle state")
                    )
    ...
    def _validate_pending_event(
        ...
        *,
        ...
        if kind == "native_attempt":
            ...
            expected_id = _digest(b"aog.receipt.v1\0", identity)
        ...
        expected_id = _digest(b"aog.receipt.v1\0", unsigned)
        if not isinstance(event_id, str) or event_id != expected_id:
            raise ControlPlaneError("pending event identity is invalid")
        require_serialized_bound()
        return event
    ...
    def _pending_event(self, kind: str, **fields: Any) -> dict[str, Any]:
        ...
        event = {
            **unsigned,
            "phase": "observed",
            "event_id": _digest(b"aog.receipt.v1\0", unsigned),
        }
    ...
    def _native_attempt_receipt(
        ...
        identity = {
            ...
            "tool_input_sha256": _digest(
                b"aog.native-input.v1\0", dict(observed_input)
            ),
        ...
        receipt = {
            **identity,
            "event_id": _digest(b"aog.receipt.v1\0", identity),
            "observation": None,
            "owner": None,
            "phase": "reserved",
            "result_sha256": None,
    ...
    def _find_native_attempt_receipt(
        ...
        *,
        ...
        prefix = f".aog-pending-s{_session_digest(self.session_id)}-"
        with acquire(
            self.root,
            STATE_ROOT_LOCK,
            timeout=_bounded_lock_timeout(self.lock_timeout),
    ...
    def _postflight_observation(
        ...
        *,
        ...
        if opaque_message:
            ...
            input_sha256 = _digest(b"aog.native-input.v1\0", dict(tool_input))
            ...
            if not isinstance(target, str):
                raise ControlPlaneError("native tool result is not AOG-owned")
            if not isinstance(previous_status, str) or not previous_status:
    ...
    def process_postflight_event(
        ...
        *,
        ...
        raise ControlPlaneError("postflight observation is not AOG-owned")
    ...
    def _resolve_state_path(self) -> Path:
        ...
        with acquire(
            ...
            matches, unsupported = _session_state_paths(self.root, self.session_id)
        if unsupported:
            raise ControlPlaneError(UNSUPPORTED_STATE_MESSAGE)
        if len(matches) > 1:
        ...
        self._state_path = (
            matches[0]
            if matches
            else self.root / f".aog-uninitialized-s{_session_digest(self.session_id)}.json"
        )
        return self._state_path
    ...
    def _workspace_hint(self) -> str:
        state = self._validate_lifecycle_state(
            _load_object(self.state_path, "aog.v1 lifecycle state"),
            expected_session=self.session_id,
        )
    ...
    def _workspace_state_candidates(
        ...
        for path in indexed:
            ...
            raw_state = _load_object(path, "aog.v1 lifecycle state")
    ...
    def _read_state(
        ...
        *,
        ...
        raw_state = _load_object(source, "aog.v1 lifecycle state")
        state = self._validate_lifecycle_state(
            raw_state,
            expected_session=self.session_id,
        )
    ...
    def _write_state(self, state: dict[str, Any]) -> None:
        ...
        target = canonical
        if target.exists():
            current_raw = _read_bounded_bytes(target, "current aog.v1 lifecycle state")
            current = self._validate_lifecycle_state(
                _decode_object(current_raw, "current aog.v1 lifecycle state"),
                expected_session=self.session_id,
            )
    ...
    def _read_plan(self, state: Mapping[str, Any]) -> dict[str, Any]:
        plan = _load_object(Path(state["plan_path"]), "aog.v1 plan artifact")
        ...
        if _digest(b"aog.plan.v1\0", unsigned) != plan["plan_id"]:
    ...
    def _read_wave(self, state: Mapping[str, Any]) -> dict[str, Any]:
        ...
        wave = _load_object(self._artifact_path("wave", wave_id), "aog.v1 wave artifact")
        ...
        if cooperative_units:
            ...
            if _digest(b"aog.wave.v1\0", cooperative_identity) != wave_id:
                raise ControlPlaneError("wave artifact digest is invalid")
        else:
            if (
            ...
            if _digest(b"aog.wave.v1\0", identity) != wave_id:
                raise ControlPlaneError("wave artifact digest is invalid")
        return wave
    ...
    def create_plan(
        ...
        *,
        ...
        plan_id = _digest(b"aog.plan.v1\0", unsigned)
        ...
        if self.state_path.exists():
            with self._coordinated_state() as state:
                ...
                raise ControlPlaneError(
                    "the current task already has AOG lifecycle proof; "
        ...
        with (
            ...
            if self.state_path.exists():
                raise ControlPlaneError(
                    "the current task already has AOG lifecycle proof; run explicit cleanup first"
                )
            with acquire(
                self.root,
                STATE_ROOT_LOCK,
                ...
                if any(
                    ...
                    raise ControlPlaneError(
                        "AOG has an unsettled native lifecycle receipt; finish restart "
    ...
    def _dispatch_record(
        ...
        *,
        ...
        dispatch_id = _digest(b"aog.dispatch.v1\0", identity)
        ...
        *,
        ...
        try:
            ...
            with self._coordinated_state(workspace_root) as state:
                ...
                wave_id = _digest(b"aog.wave.v1\0", digest_identity)
                if cooperative:
                    assert canonical_baseline is not None
                    wave = {
                        **wave_identity,
    ...
    def _record_cooperative_result(
        ...
        try:
            # A canonical write by a child is detected here; AOG never attempts
        ...
        try:
            with acquire(
                ...
                for path in _state_json_paths(self.root):
                    raw = _read_bounded_bytes(path, "aog.v1 orphan liveness state")
                    ...
                    if scanned_bytes > MAX_ISOLATION_LIVENESS_SCAN_BYTES:
                        ...
                        return 0
                    state = self._validate_lifecycle_state(
                        _decode_object(raw, "aog.v1 orphan liveness state")
                    )
    ...
    def status(self) -> dict[str, Any]:
        with self._coordinated_state() as state:
            ...
            return {
                ...
                "epoch": state["epoch"],
                "plan_id": state["plan_id"],
                "protocol": "aog.status.v1",
                "state": self._overall_state(state, plan),
            }

    def cleanup(self) -> int:
        """Remove only this task's inactive state and immutable artifacts."""
    ...
    def terminal_proof(self, owner: str, result: Mapping[str, Any]) -> dict[str, Any]:
        ...
        with self._coordinated_state() as state:
            ...
            if (
                ...
                raise ControlPlaneError("AOG child is not proof-backed terminal work")
            return {
                "dispatch_id": dispatch["dispatch_id"],
                "role": dispatch["role"],
                "state": dispatch["state"],
    ...
    def stop_reason(self) -> str | None:
        ...
        with self._coordinated_state() as state:
            ...
            if any(
                ...
                return "AOG child work is still active; wait for its native terminal event."
            return None
    ...
    def pending_event_reason(self) -> str | None:
        ...
        if not pending:
            return None
        return (
            "AOG has an unsettled native lifecycle receipt; finish restart recovery "
            "before stopping."
        )
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(description="Compile and operate one compact aog.v1 plan.")
    ...
    try:
        if args.command == "prepare":
            ...
            if args.command == "next":
                ...
                result = {"interrupted": control.restart(), "protocol": "aog.restart.v1"}
            elif args.command == "cleanup":
                result = {"protocol": "aog.cleanup.v1", "removed": control.cleanup()}
            else:
                result = control.status()
    except (
        ControlPlaneError,
