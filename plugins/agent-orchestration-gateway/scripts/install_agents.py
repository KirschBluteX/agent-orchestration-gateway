#!/usr/bin/env python3
"""Install and diagnose the two model-neutral aog.v1 native Agent profiles."""
...
PLUGIN_ID = "agent-orchestration-gateway@agent-orchestration-gateway"
PLUGIN_VERSION = "0.10.0"
PLUGIN_RELEASE = PLUGIN_VERSION
PROFILES = {
    "read": ("aog-read-leaf.toml", "aog_read_leaf"),
    "write": ("aog-write-leaf.toml", "aog_write_leaf"),
}
...
HOST_HOOK_CONTRACT: tuple[tuple[str, str | None], ...] = (
    ...
    ("stop", None),
    ("subagentStop", "^(aog_read_leaf|aog_write_leaf)$"),
)
...
def _selected(names: list[str] | None) -> list[str]:
    ...
    if any(name not in PROFILES for name in selected):
        raise InstallError("unknown AOG profile")
    return selected
...
def doctor(
    ...
    *,
    ...
    if check(target) != 0:
        errors.append("installed AOG profiles are not ready")
    try:
        ...
        if any(counts[event] < count for event, count in expected_events.items()):
            errors.append("Codex did not discover every required AOG Hook")
        if any(counts[event] > count for event, count in expected_events.items()):
            errors.append("Codex discovered duplicate AOG Hook definitions")
        if unknown_definitions:
            errors.append("Codex discovered unknown AOG Hook definitions")
        if canonical_hook_schema is not None:
            ...
            if any(any(field in item for field in schema_fields) for item in discovered):
                try:
                    ...
                    errors.append("Codex returned incomplete AOG Hook definitions")
                else:
        ...
        if any(
            ...
            errors.append("AOG Hooks are not enabled and trusted; review them in /hooks")
        ...
        errors.append(f"AOG Hook trust could not be verified: {error}")
    try:
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(description="Install or diagnose aog.v1 native Agent profiles.")
