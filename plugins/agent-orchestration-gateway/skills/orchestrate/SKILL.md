...
name: orchestrate
description: >-
  Compile and route closed, typed-scope Codex work through one AOG prepare command while Primary
  keeps intent and final acceptance. Use for delegable repository work; invoke native Agent
  actions only when returned by AOG.
---

# Orchestrate with AOG
...
Send one schema-validated `aog.delegation.v1` request to this command. Give every node explicit
...
`aog.planner-proposal.v1` DAG input.
...
planner or child. A pre-existing `aog.planner-proposal.v1` value is only schema-validated DAG input,
not a route, lifecycle, contract file, or permission to bypass AOG.

Invoke every returned `tool_name` with only its exact `tool_input`. Do not expose model or
cost reasoning in normal output, alter a returned input, or add an unreturned native action. AOG
...
scopes. It may work on disjoint scopes, perform AOG lifecycle or status checks, or wait. Re-enter
...
Use `$agent-orchestration-gateway:manage-aog` only for installation, trust, lifecycle commands,
or cleanup.
