...
HOOKS = ROOT / "plugins" / "agent-orchestration-gateway" / "hooks"
SCRIPTS = ROOT / "plugins" / "agent-orchestration-gateway" / "scripts"
sys.path.insert(0, str(HOOKS))
sys.path.insert(0, str(SCRIPTS))

import aog_hook  # noqa: E402
...
class HookTests(unittest.TestCase):
    def test_uuid_subagent_stop_maps_owner_from_trusted_rollout(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(aog_hook, "_sessions_root", return_value=sessions.resolve()):
                self.assertEqual(
                    aog_hook._owner(
                        {
                            "agent_id": agent_id,
    ...
    def test_uuid_owner_mapping_replays_after_temporary_rollout_io_failure(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(aog_hook, "_sessions_root", return_value=sessions.resolve()),
                patch.object(
                    aog_hook,
                    "first_record",
                    side_effect=RolloutUnavailable("rollout is temporarily unavailable"),
                ),
                self.assertRaises(aog_hook.ControlPlaneUnavailable),
            ):
                aog_hook._owner(payload)
    ...
    def test_protected_subagent_result_is_recovered_from_trusted_rollout(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(aog_hook, "_sessions_root", return_value=sessions.resolve()),
                patch.object(aog_hook, "_control", return_value=Control()),
            ):
                outcome = aog_hook.evaluate(
                    {
                        "agent_id": agent_id,
                        "agent_transcript_path": str(rollout),
    ...
    def test_protected_subagent_stop_uses_latest_rollout_result_for_reused_owner(self) -> None:
        ...
        historical = "AOG_RESULT aog.v1\n{\"status\":\"historical\"}"
        current = "AOG_RESULT aog.v1\n{\"status\":\"current\"}"
        with tempfile.TemporaryDirectory() as directory:
            sessions = Path(directory) / "sessions"
            sessions.mkdir()
            rollout = sessions / f"rollout-test-{agent_id}.jsonl"
    ...
    def test_protected_subagent_stop_fences_non_aog_latest_rollout_final(self) -> None:
    ...
    def test_missing_protected_transcript_result_fences_managed_owner(self) -> None:
        ...
        with (
            patch.object(aog_hook, "_control", return_value=Control()),
            patch.object(aog_hook, "_owner", return_value="/root/worker_n01"),
            patch.object(
                aog_hook,
                "_result_from_transcript",
                side_effect=aog_hook.ControlPlaneError(
                    "protected child result has no final AOG_RESULT"
                ),
            ),
        ):
            outcome = aog_hook.evaluate(
    ...
    def test_opaque_followup_is_delegated_to_the_control_plane(self) -> None:
        ...
        with patch.object(aog_hook, "_control", return_value=Control()):
            outcome = aog_hook.evaluate(payload)
    ...
    def test_opaque_followup_control_plane_rejection_blocks(self) -> None:
        class Control:
            ...
            def preflight_opaque_followup(_payload: object) -> None:
                raise aog_hook.ControlPlaneError("no prepared opaque follow-up")
    ...
    def test_unmappable_owner_closes_unresolved_leases(self) -> None:
        ...
        with (
            ...
            patch.object(
                aog_hook,
                "_owner",
                side_effect=aog_hook.ControlPlaneError("missing trusted owner"),
            ),
        ):
            outcome = aog_hook.evaluate(payload)
        ...
        outcome = aog_hook.evaluate(
            {
                "hook_event_name": "PreToolUse",
                "session_id": "hook-session",
                "tool_input": {"message": "AOG_NATIVE_BYPASS v1\ninspect one image"},
                "tool_name": "spawn_agent",
                "tool_use_id": "call",
            }
        )
    ...
    def test_pretool_internal_budget_is_below_host_timeout(self) -> None:
        ...
        self.assertLess(aog_hook.PRETOOL_INTERNAL_BUDGET_SECONDS, host_timeout)
        control = aog_hook._control(
    ...
    def test_posttool_internal_budget_is_below_host_timeout(self) -> None:
        ...
        self.assertLess(aog_hook.POSTTOOL_INTERNAL_BUDGET_SECONDS, host_timeout)
        control = aog_hook._control(
            {"hook_event_name": "PostToolUse", "session_id": "budgeted-post-hook"}
        )
        self.assertLess(control.lock_timeout, aog_hook.POSTTOOL_INTERNAL_BUDGET_SECONDS)
    ...
    def test_subagent_stop_budget_leaves_host_settlement_reserve(self) -> None:
        ...
        self.assertLess(aog_hook.SUBAGENT_STOP_INTERNAL_BUDGET_SECONDS, host_timeout)
        control = aog_hook._control(
    ...
    def test_session_and_stop_budgets_are_below_host_timeouts(self) -> None:
        ...
        self.assertLess(aog_hook.SESSION_START_INTERNAL_BUDGET_SECONDS, session_timeout)
        self.assertLess(aog_hook.STOP_INTERNAL_BUDGET_SECONDS, stop_timeout)
            ...
            outcome = aog_hook.evaluate(
                {
                    "hook_event_name": "PostToolUse",
                    "session_id": "hook-session",
                    "tool_input": {"message": "AOG_TASK aog.v1\n{}"},
                    "tool_name": "spawn_agent",
    ...
    def test_followup_task_routes_a_new_aog_task_to_reuse_preflight(self) -> None:
        calls: list[object] = []
    ...
    def test_stop_blocks_only_the_first_stop_event_while_work_is_active(self) -> None:
        ...
        with patch.object(aog_hook, "_control", return_value=Control()):
            first = aog_hook.evaluate(
                {"hook_event_name": "Stop", "stop_hook_active": False}
            )
            repeated = aog_hook.evaluate(
    ...
    def test_subagent_stop_records_first_result_and_prevents_other_continuations(self) -> None:
        ...
        with (
            ...
            outcome = aog_hook.evaluate(
                {
                    "hook_event_name": "SubagentStop",
                    "last_assistant_message": "AOG_RESULT aog.v1\n{}",
                    "stop_hook_active": False,
                }
            )
        self.assertEqual(calls, [("/root/worker_n01", "AOG_RESULT aog.v1\n{}")])
        self.assertEqual(outcome, {"continue": False})
    ...
    def test_arbitrary_assistant_error_text_is_not_treated_as_typed_native_failure(self) -> None:
        ...
        class Control:
            ...
            def process_result_event(owner: str, result: object) -> None:
                calls.append((owner, result))
                raise ValueError("not a AOG result")
    ...
    def test_interrupt_is_validated_then_settled_by_post_tool_use(self) -> None:
        ...
        with patch.object(aog_hook, "_control", return_value=Control()):
            self.assertEqual(aog_hook.evaluate(pre), {})
            self.assertEqual(aog_hook.evaluate(post), {})
    ...
    def test_unmanaged_post_tool_and_interrupt_are_noops(self) -> None:
        ...
        with patch.object(aog_hook, "_control", return_value=Control()):
            post = aog_hook.evaluate(
                {
                    "hook_event_name": "PostToolUse",
                    "tool_input": {"message": "ordinary task"},
                }
            )
            interrupt = aog_hook.evaluate(
                {
                    "hook_event_name": "PreToolUse",
            ...
            interrupt_post = aog_hook.evaluate(
                {
                    "hook_event_name": "PostToolUse",
    ...
    def test_ambiguous_session_state_blocks_raw_managed_message(self) -> None:
        class Control:
            @staticmethod
            def owner_is_managed(_owner: str) -> bool:
                raise aog_hook.ControlPlaneError(
    ...
    def test_unsupported_artifacts_fail_closed_for_one_shot_hook_events(self) -> None:
        class Control:
            @staticmethod
            def process_restart_event(_source: str) -> int:
                raise aog_hook.ControlPlaneUnavailable(
                    "unsupported AOG state or artifact"
                )
        ...
        self.assertIn("unsupported AOG state", session["systemMessage"])
        self.assertIn("unsupported AOG state", post["reason"])
